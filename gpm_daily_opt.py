"""
NASA GPM DAILY TO MONTHLY PROCESSING WORKFLOW - CORRECTED UNITS
Downloads daily NetCDF files (mm/day), processes to monthly GeoTIFFs (mm/month)
"""
...
class GPMDailyToMonthlyWorkflow:
    ...
    def __init__(self, base_dir=None):
        ...
        if base_dir is None:
            self.base_dir = Path.home() / "Documents" / "Benjamin" / "GPM" / "GPM_gee" / "GPM_NASA_Daily_corrected"
        else:
    ...
    def download_daily_file(self, year, month, day, max_retries=3):
        """Download a single daily file - FIXED to use self.opener"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Use opener directly instead of urlretrieve for thread safety
                ...
                if e.code == 404:
                    ...
                    print(f"\n    ❌ Day {day:02d}: Authentication failed (401)")
                    # Don't retry on auth failure
                    break
                elif e.code == 403:
                    print(f"\n    ❌ Day {day:02d}: Access forbidden (403)")
                    break
                else:
                    print(f"\n    ❌ Day {day:02d}: HTTP Error {e.code}: {e.reason}")
            except Exception as e:
    ...
    def download_month_daily_files(self, year, month, max_workers=3):
        """Download all daily files for a month in parallel - REDUCED workers for stability"""
        ...
        # Use ThreadPoolExecutor for parallel downloads (reduced workers)
    ...
    # =========================================================================
    # PROCESSING METHODS: DAILY TO MONTHLY - CORRECTED UNITS
    # =========================================================================
    
    def process_daily_file(self, nc4_path):
        """Process a single daily NetCDF file and extract precipitation - UNITS CORRECTED"""
        try:
            ...
            # Priority order based on GPM documentation
            precip_vars_to_try = [
                'precipitation',        # Daily mean precipitation rate (mm/day)
                'precipitationCal',    # Older name for same variable
                'MWprecipitation',     # Microwave-only precipitation (mm/day)
                'HQprecipitation',     # Older name for MWprecipitation
                'precip',
                'precipitationRate',
                'IRprecipitation',
                'precipCal'
            ...
            # GPM daily data is in mm/day, NOT mm/hr
            precip_mm_day = precip_clean
            conversion_note = ""
            
            # Check units and validate
            units_lower = str(units).lower()
            if 'mm/day' in units_lower or 'mm d-1' in units_lower or 'mm day-1' in units_lower:
                print(f"  ✅ Units confirmed: {units} (mm/day) - no conversion needed")
                conversion_note = " (already mm/day)"
            elif 'mm/hr' in units_lower or 'mm h-1' in units_lower or 'mm hr-1' in units_lower:
                # This should NOT happen for daily products, but handle it
                print(f"  ⚠️ Unexpected units: {units} - converting mm/hr → mm/day")
                precip_mm_day = precip_clean * 24
                conversion_note = " (converted mm/hr→mm/day)"
            elif 'mm' in units_lower and ('day' not in units_lower and 'hr' not in units_lower):
                # Generic 'mm' - assume mm/day for daily products
                print(f"  ⚠️ Generic units '{units}' - assuming mm/day (standard for daily products)")
                conversion_note = " (assumed mm/day)"
            else:
                # Unknown units - assume mm/day based on product type
                print(f"  ⚠️ Unknown units: {units}")
                print(f"     Assuming mm/day (standard for GPM 3IMERGDF daily products)")
                conversion_note = " (assumed mm/day - standard for daily products)"
    ...
    def process_month_from_daily(self, year, month, daily_files=None):
        """Process all daily files for a month and create monthly aggregate - CORRECTED"""
        ...
        print(f"  📝 Note: Daily data is in mm/day, summing to get mm/month")
        ...
        try:
            ...
            # Sum across time dimension (monthly total in mm/month)
            # Since data is already in mm/day, summing gives mm/month
            ...
            print(f"\n  📊 MONTHLY STATISTICS:")
            print(f"    Min: {np.nanmin(monthly_total):.1f} mm/month")
            print(f"    Max: {np.nanmax(monthly_total):.1f} mm/month")
            print(f"    Mean: {np.nanmean(monthly_total):.1f} mm/month")
    ...
    def _save_monthly_geotiff(self, data, lat, lon, output_path, year, month, daily_count):
        ...
        try:
            ...
            print(f"    Data range: {np.nanmin(data):.1f} to {np.nanmax(data):.1f} mm/month")
            ...
            with rasterio.open(
                ...
                dst.update_tags(
                    ...
                    SOURCE_UNITS='mm/day (summed to mm/month)',
                    DAILY_FILES_USED=str(daily_count),
                    RESOLUTION=f'{pixel_width:.3f} degree',
                    YEAR=str(year),
                    MONTH=str(month),
                    PROCESSING_DATE=dt.now().isoformat(),
                    PROCESSING_NOTE='Daily data in mm/day summed to monthly total in mm/month'
                )
                
                dst.set_band_description(1, f'Monthly Precipitation {year}-{month:02d} (mm/month)')
    ...
    def _create_monthly_metadata(self, metadata_path, year, month, daily_files_processed, stats):
        ...
        try:
            with open(metadata_path, 'w') as f:
                ...
                f.write("UNITS:\n")
                f.write(f"  Source data: mm/day (daily mean precipitation)\n")
                f.write(f"  Output data: mm/month (sum of daily values)\n\n")
                ...
                f.write("  2. Extract precipitation data (precipitation variable)\n")
                f.write("  3. Remove time dimension (squeeze)\n")
                f.write("  4. Verify units are mm/day (no conversion needed)\n")
                f.write("  5. Transpose [lon, lat] → [lat, lon] if needed\n")
                f.write("  6. Sum daily precipitation to monthly total (mm/day → mm/month)\n")
                f.write("  7. Apply WGS84 georeferencing\n")
                f.write("  8. Save as compressed GeoTIFF\n\n")
                
                f.write("VARIABLE INFORMATION:\n")
                f.write("  precipitation: Daily mean precipitation rate (mm/day)\n")
                f.write("    - Combined microwave-IR estimate\n")
                f.write("    - Formerly called precipitationCal\n")
                f.write("  MWprecipitation: High Quality MW precipitation (mm/day)\n")
                f.write("    - Formerly called HQprecipitation\n")
    ...
    def download_year_range(self, start_year=1998, end_year=2025):
        """Download daily files for a range of years"""
...
if __name__ == "__main__":
    ...
    print("\n" + "="*70)
    print("🌧️ NASA GPM DAILY TO MONTHLY WORKFLOW - UNITS CORRECTED")
    print("="*70)
    print("Daily data: mm/day → Monthly data: mm/month (sum)")
    print("="*70)
